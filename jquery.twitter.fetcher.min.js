...
f.removeClass("loading").find("ul").append(d)}},g={init:function(a){b.extend(c,a);f=b(this[0]);f.twitter_fetcher("update")},update:function(){c.user||b.error("User is a mandatory option.");f.empty().addClass("loading").append(b("<ul>"));c.list?b.getJSON("https://api.twitter.com/1/lists/statuses.json?callback=?",{slug:c.list,owner_screen_name:c.user,page:1,per_page:c.limit},h):b.getJSON("https://api.twitter.com/1/statuses/user_timeline.json?callback=?",{screen_name:c.user,page:1,per_page:c.limit},
h)}};b.fn.twitter_fetcher=function(a){if(g[a])return g[a].apply(this,Array.prototype.slice.call(arguments,1));else if(typeof a==="object"||!a)return g.init.apply(this,arguments);else b.error("Method "+a+" does not exist on jQuery.twitter.fetcher")}})(jQuery);
